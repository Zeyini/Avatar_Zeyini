...
# 🆕 watchdog for the QML auto-reloader 👾
...
# 🆕 Helper class that reloads QML whenever you save main.qml 👾
class QmlReloader(FileSystemEventHandler):
    def __init__(self, engine, qml_path):
        self.engine = engine
        self.qml_path = qml_path

    def on_modified(self, event):
        if event.src_path.endswith("main.qml"):
            print("🔁 Detected change in main.qml — reloading UI…")
            try:
                self.engine.clearComponentCache()
                self.engine.load(QUrl.fromLocalFile(self.qml_path))
                print("✅ QML reloaded successfully.")
            except Exception as e:
                print(f"⚠️ Error reloading QML: {e}")
...
if __name__ == "__main__":
   ...
   # Load QML
    ...
    # 🆕 Start watching for QML changes 👾  
    from pathlib import Path
    qml_path = str(qml_file)
    reloader = QmlReloader(engine, qml_path)
    observer = Observer()
    observer.schedule(reloader, Path(qml_path).parent, recursive=False)
    observer.start()
    print("👀 Watching for main.qml changes...")

    # Delay heavy plot conversion so UI opens instantly
    from PySide6.QtCore import QTimer

    def delayed_plot_conversion():
        try:
            backend.convert_pdfs_to_images()
        except Exception as e:
            print(f"Error converting PDFs: {str(e)}")

    # Run heavy task 1s after window shows
    QTimer.singleShot(1000, delayed_plot_conversion)
    ...
    # 🆕 Gracefully stop the dogwatcher on exit
    exit_code = app.exec()
    observer.stop()
    observer.join()
    sys.exit(exit_code)
